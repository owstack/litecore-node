...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with Ltc',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@owstack/ltc-lib': '^' + ltcLib.version,
    '@owstack/ltc-node': version
  }
};
...
/**
 * Will create a base Ltc Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/ltc-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Ltc Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the ltc node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the ltc-node directory and configuration
