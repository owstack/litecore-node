...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and ltc-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/owstack/ltc-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed ltc-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ltc-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in ltc-node services directory
    ...
    if (servicePackage.ltcNode) {
      serviceModule = service.moduleName + '/' + servicePackage.ltcNode;
 ...
 * @param {Node} node - The Ltc Node instance
 */
