...
/**
 * Will return the path and ltc-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'ltc-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'ltc-node.json'))
  };
}
