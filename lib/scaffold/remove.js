...
/**
 * Will remove a service from ltc-node.json
...
/**
 * Will remove the Node.js service and from the ltc-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The ltc-node configuration directory
...
function remove(options, done) {
  ...
  var ltcNodeConfigPath = path.resolve(configPath, 'ltc-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(ltcNodeConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a ltc-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to ltc-node.json
        removeConfig(ltcNodeConfigPath, service, next);
      });
    }, done
  );
}
